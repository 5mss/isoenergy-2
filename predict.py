...
start = time.time()
scaler_feature = preprocessing.StandardScaler(copy=False)
scaler_feature.fit(data)
...
start = time.time()
with h5py.File('Y_pred.h5', 'w') as xp:
    xp['feature'] = X_pred
print('Saving complete. Time used: ', time.time() - start)
print('Loading models...')
nnModel = joblib.load('nnModel')
...
print('Saving...')
with h5py.File('answer.h5', 'w') as opt:
    for i in range(900):
